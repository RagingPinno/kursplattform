...
function Login() {
  ...
  const login = async () => {
    try {
      ...
      // onAuthStateChanged i App.jsx kommer att upptäcka inloggningen.
      // Nu omdirigerar vi användaren.
      
      // ✅ Hämta den sparade sökvägen från sessionStorage
      const redirectPath = sessionStorage.getItem('redirectPath');
      // Ta bort den så den inte ligger kvar
      sessionStorage.removeItem('redirectPath');
      
      // Omdirigera till den sparade sidan, eller till startsidan som standard
      ...
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8 text-center">
        <div className="mb-8">
          <div className="w-20 h-20 mx-auto bg-indigo-100 rounded-full flex items-center justify-center mb-4">
            <svg className="w-10 h-10 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path></svg>
          </div>
          <h1 className="text-4xl font-bold text-gray-900">Välkommen!</h1>
          <p className="text-gray-500 mt-2">Logga in för att hantera dina kurser.</p>
        </div>
        <button 
          onClick={login} 
